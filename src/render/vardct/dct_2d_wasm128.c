...
static void dct4_forward_lanes(jxl_lane out[4], const jxl_lane in[4]) {
    jxl_wasm128_dct4_forward_lanes((jxl_wasm128_lane *)out, (const jxl_wasm128_lane *)in);
}

static void dct4_inverse_lanes(jxl_lane out[4], const jxl_lane in[4]) {
    jxl_wasm128_dct4_inverse_lanes((jxl_wasm128_lane *)out, (const jxl_wasm128_lane *)in);
}
...
static void dct_lanes(jxl_lane *io, jxl_lane *scratch, size_t n, jxl_dct_direction direction) {
    ...
    if (n == 2) {
        const jxl_lane tmp0 = wasm_f32x4_add(io[0], io[1]);
        const jxl_lane tmp1 = wasm_f32x4_sub(io[0], io[1]);
        if (direction == JXL_DCT_FORWARD) {
            io[0] = wasm_f32x4_mul(tmp0, half);
            io[1] = wasm_f32x4_mul(tmp1, half);
        } else {
            io[0] = tmp0;
            io[1] = tmp1;
        }
    ...
    if (n == 8) {
        if (direction == JXL_DCT_FORWARD) {
            jxl_wasm128_dct8_forward_lanes((jxl_wasm128_lane *)io);
        } else {
            jxl_wasm128_dct8_inverse_lanes((jxl_wasm128_lane *)io);
        }
        return;
    }
    ...
    if (direction == JXL_DCT_FORWARD) {
        ...
        for (idx = 0; idx < n / 2; ++idx) {
            input0[idx] = wasm_f32x4_mul(wasm_f32x4_add(io[idx], io[n - idx - 1]), half);
            input1[idx] =
                wasm_f32x4_mul(wasm_f32x4_sub(io[idx], io[n - idx - 1]), lane_splat(sec[idx] / 2.0f));
        }
        ...
        input1[0] = wasm_f32x4_mul(input1[0], sqrt2);
        for (idx = 0; idx < n / 2 - 1; ++idx) {
            io[idx * 2 + 1] = wasm_f32x4_add(input1[idx], input1[idx + 1]);
        }
        io[n - 1] = input1[n / 2 - 1];
    } else {
        size_t i;
        ...
        jxl_lane *input1 = scratch + n / 2;
        for (i = 1; i < n / 2; ++i) {
            const size_t rev = n / 2 - i;
            input0[rev] = io[rev * 2];
            input1[rev] = wasm_f32x4_add(io[rev * 2 + 1], io[rev * 2 - 1]);
        }
        input0[0] = io[0];
        ...
        for (idx = 0; idx < n / 2; ++idx) {
            const jxl_lane r = wasm_f32x4_mul(input1[idx], lane_splat(sec[idx]));
            io[idx] = wasm_f32x4_add(input0[idx], r);
            io[n - idx - 1] = wasm_f32x4_sub(input0[idx], r);
        }
    }
}
...
static void column_dct_lane(jxl_lane_subgrid *io, jxl_lane *scratch, jxl_dct_direction direction) {
    ...
    for (x = 0; x < width; ++x) {
        ...
        for (y = 0; y < height; y += JXL_LANE_SIZE) {
            ...
            jxl_wasm128_transpose_lane((jxl_wasm128_lane *)block);
            for (dy = 0; dy < JXL_LANE_SIZE; ++dy) {
...
static void dct8_column_lanes(jxl_lane_subgrid *io, size_t x, jxl_dct_direction direction) {
    ...
    if (direction == JXL_DCT_FORWARD) {
        jxl_wasm128_dct8_forward_lanes((jxl_wasm128_lane *)col);
    } else {
        jxl_wasm128_dct8_inverse_lanes((jxl_wasm128_lane *)col);
    }
    for (y = 0; y < 8; ++y) {
...
static void dct8x8_lane(jxl_lane_subgrid *io, jxl_dct_direction direction) {
    ...
    for (y = 0; y < 8; ++y) {
        ...
        jxl_lane out_r;
        if (direction == JXL_DCT_FORWARD) {
            jxl_wasm128_dct8_vec_forward(vl, vr, &out_l, &out_r);
        } else {
            jxl_wasm128_dct8_vec_inverse(vl, vr, &out_l, &out_r);
        }
...
int jxl_dct_2d_wasm128(jxl_context *alloc, jxl_subgrid_f32 io, jxl_dct_direction direction) {
