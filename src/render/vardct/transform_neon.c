...
jxl_inline jxl_neon_lane shuffle_a0b0(jxl_neon_lane a, jxl_neon_lane b) {
    return vuzpq_f32(a, b).val[0];
}

jxl_inline jxl_neon_lane shuffle_a1b1(jxl_neon_lane a, jxl_neon_lane b) {
...
static void transform_dct4_neon(jxl_subgrid_f32 coeff) {
    size_t y2;
    size_t y;
    jxl_neon_lane scratch_0[4];
    jxl_neon_lane scratch_1[4];
    aux_idct2_in_place_2(coeff);

    for (y2 = 0; y2 < 4; ++y2) {
        const float *row_ptr = jxl_subgrid_f32_row_mut(coeff, y2 * 2);
        jxl_neon_lane a = vld1q_f32(row_ptr);
        jxl_neon_lane b = vld1q_f32(row_ptr + 4);
        scratch_0[y2] = shuffle_a0b0(a, b);
        scratch_1[y2] = shuffle_a1b1(a, b);
    }

    jxl_neon_transpose_lane(scratch_0);
    jxl_neon_transpose_lane(scratch_1);
    jxl_neon_dct4_inverse_4(scratch_0);
    jxl_neon_dct4_inverse_4(scratch_1);
    for (y2 = 0; y2 < 4; ++y2) {
        float *row_ptr = jxl_subgrid_f32_row_mut(coeff, y2);
        vst1q_f32(row_ptr, jxl_neon_dct4_vec_inverse(scratch_0[y2]));
        vst1q_f32(row_ptr + 4, jxl_neon_dct4_vec_inverse(scratch_1[y2]));
    ...
    for (y = 0; y < 4; ++y) {
        ...
        vst1q_f32(row_ptr, jxl_neon_dct4_vec_inverse(scratch_0[y]));
        vst1q_f32(row_ptr + 4, jxl_neon_dct4_vec_inverse(scratch_1[y]));
    }
}

static void transform_dct4x8_neon(jxl_subgrid_f32 coeff, int transpose) {
    ...
    if (transpose) {
        ...
        for (y2 = 0; y2 < 4; ++y2) {
            ...
            jxl_neon_lane a = vld1q_f32(row_ptr);
            jxl_neon_lane b = vld1q_f32(row_ptr + 4);
            jxl_neon_dct8_vec_inverse(a, b, &scratch_0[y2], &scratch_1[y2]);
        }
        ...
        for (y2 = 0; y2 < 4; ++y2) {
            ...
            jxl_neon_lane a = vld1q_f32(row_ptr);
            jxl_neon_lane b = vld1q_f32(row_ptr + 4);
            jxl_neon_dct8_vec_inverse(a, b, &out_l, &out_r);

            float *dst = jxl_subgrid_f32_row_mut(coeff, y2);
            vst1q_f32(dst, scratch_0[y2]);
            dst = jxl_subgrid_f32_row_mut(coeff, y2 + 4);
            vst1q_f32(dst, scratch_1[y2]);
        ...
        {
            jxl_neon_lane tmp = scratch_0[1];
            scratch_0[1] = scratch_0[2];
            scratch_0[2] = tmp;
        }
        {
            jxl_neon_lane tmp = scratch_1[1];
        ...
        for (y = 0; y < 4; ++y) {
            ...
            vst1q_f32(row_ptr + 4, scratch_0[y]);
            row_ptr = jxl_subgrid_f32_row_mut(coeff, y + 4);
            vst1q_f32(row_ptr + 4, scratch_1[y]);
        }
    } else {
        size_t y2;
        size_t y;
        ...
        for (y2 = 0; y2 < 4; ++y2) {
            ...
            vst1q_f32(row_ptr + 4, scratch_1[y2]);
...
int jxl_render_transform_varblock_neon(jxl_allocator_state *alloc, jxl_subgrid_f32 coeff,
    ...
    case JXL_TRANSFORM_DCT4:
        ...
        transform_dct4_neon(coeff);
        return 1;
    case JXL_TRANSFORM_DCT4X8:
        if (!subgrid_simd_eligible(coeff, 8, 8)) {
            return 0;
        }
        transform_dct4x8_neon(coeff, 0);
        return 1;
    case JXL_TRANSFORM_DCT8X4:
        if (!subgrid_simd_eligible(coeff, 8, 8)) {
            return 0;
        }
        transform_dct4x8_neon(coeff, 1);
        return 1;
    ...
    default:
        ...
        if (!jxl_dct_2d_aarch64_neon(alloc, coeff, JXL_DCT_INVERSE)) {
            return 0;
        }
        return 1;
    }
