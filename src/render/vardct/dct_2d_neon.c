...
static void dct4_forward_lanes(jxl_lane out[4], const jxl_lane in[4]) {
    jxl_neon_dct4_forward_lanes((jxl_neon_lane *)out, (const jxl_neon_lane *)in);
}

static void dct4_inverse_lanes(jxl_lane out[4], const jxl_lane in[4]) {
    jxl_neon_dct4_inverse_lanes((jxl_neon_lane *)out, (const jxl_neon_lane *)in);
}
...
static void dct_lanes(jxl_lane *io, jxl_lane *scratch, size_t n, jxl_dct_direction direction) {
    ...
    if (n == 2) {
        const jxl_lane tmp0 = vaddq_f32(io[0], io[1]);
        const jxl_lane tmp1 = vsubq_f32(io[0], io[1]);
        if (direction == JXL_DCT_FORWARD) {
            io[0] = vmulq_f32(tmp0, half);
            io[1] = vmulq_f32(tmp1, half);
        } else {
            io[0] = tmp0;
            io[1] = tmp1;
        }
    ...
    if (n == 8) {
        if (direction == JXL_DCT_FORWARD) {
            jxl_neon_dct8_forward_lanes((jxl_neon_lane *)io);
        } else {
            jxl_neon_dct8_inverse_lanes((jxl_neon_lane *)io);
        }
        return;
    }
    ...
    if (direction == JXL_DCT_FORWARD) {
        ...
        for (idx = 0; idx < n / 2; ++idx) {
            input0[idx] = vmulq_f32(vaddq_f32(io[idx], io[n - idx - 1]), half);
            input1[idx] =
                vmulq_f32(vsubq_f32(io[idx], io[n - idx - 1]), lane_splat(sec[idx] / 2.0f));
        }
        ...
        input1[0] = vmulq_f32(input1[0], sqrt2);
        for (idx = 0; idx < n / 2 - 1; ++idx) {
            io[idx * 2 + 1] = vaddq_f32(input1[idx], input1[idx + 1]);
        }
        io[n - 1] = input1[n / 2 - 1];
    } else {
        size_t i;
        ...
        for (i = 1; i < n / 2; ++i) {
            ri = n / 2 - i;
            input0[ri] = io[ri * 2];
            input1[ri] = vaddq_f32(io[ri * 2 + 1], io[ri * 2 - 1]);
        }
        input0[0] = io[0];
        ...
        for (idx = 0; idx < n / 2; ++idx) {
            const jxl_lane r = vmulq_f32(input1[idx], lane_splat(sec[idx]));
            io[idx] = vaddq_f32(input0[idx], r);
            io[n - idx - 1] = vsubq_f32(input0[idx], r);
        }
    }
}
...
static void column_dct_lane(jxl_lane_subgrid *io, jxl_lane *scratch, jxl_dct_direction direction) {
    ...
    for (x = 0; x < width; ++x) {
        ...
        for (y = 0; y < height; y += JXL_LANE_SIZE) {
            ...
            jxl_neon_transpose_lane((jxl_neon_lane *)block);
            for (dy = 0; dy < JXL_LANE_SIZE; ++dy) {
...
static void dct8_column_lanes(jxl_lane_subgrid *io, size_t x, jxl_dct_direction direction) {
    ...
    if (direction == JXL_DCT_FORWARD) {
        jxl_neon_dct8_forward_lanes((jxl_neon_lane *)col);
    } else {
        jxl_neon_dct8_inverse_lanes((jxl_neon_lane *)col);
    }
    for (y = 0; y < 8; ++y) {
...
static void dct8x8_lane(jxl_lane_subgrid *io, jxl_dct_direction direction) {
    ...
    for (y = 0; y < 8; ++y) {
        ...
        jxl_lane out_r;
        if (direction == JXL_DCT_FORWARD) {
            jxl_neon_dct8_vec_forward(vl, vr, &out_l, &out_r);
        } else {
            jxl_neon_dct8_vec_inverse(vl, vr, &out_l, &out_r);
        }
