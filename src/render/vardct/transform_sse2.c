...
static void transform_dct4_sse2(jxl_subgrid_f32 coeff) {
    size_t y2;
    size_t y;
...
static void transform_dct4x8_sse2(jxl_subgrid_f32 coeff, int transpose) {
...
int jxl_render_transform_varblock_sse2(jxl_allocator_state *alloc, jxl_subgrid_f32 coeff,
    ...
    case JXL_TRANSFORM_DCT4:
        ...
        transform_dct4_sse2(coeff);
        return 1;
    case JXL_TRANSFORM_DCT4X8:
        if (!subgrid_simd_eligible(coeff, 8, 8)) {
            return 0;
        }
        transform_dct4x8_sse2(coeff, 0);
        return 1;
    case JXL_TRANSFORM_DCT8X4:
        if (!subgrid_simd_eligible(coeff, 8, 8)) {
            return 0;
        }
        transform_dct4x8_sse2(coeff, 1);
        return 1;
