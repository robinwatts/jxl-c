...
__m128 jxl_color_linear_to_pq_vec_x86_sse2(__m128 v, float intensity_target) {
    ...
    const __m128 v_small =
        jxl_fastmath_rational_eval5_x86_sse2(a_1_4, k_inv_eotf_p_small, k_inv_eotf_q_small);
    const __m128 v_large = jxl_fastmath_rational_eval5_x86_sse2(a_1_4, k_inv_eotf_p, k_inv_eotf_q);
    const __m128 is_small = _mm_cmplt_ps(a, _mm_set1_ps(1e-4f));
    const __m128 y = _mm_or_ps(_mm_andnot_ps(is_small, v_large), _mm_and_ps(is_small, v_small));
...
__m128 jxl_color_pq_to_linear_vec_x86_sse2(__m128 v, float intensity_target) {
    ...
    const __m128 sign = _mm_and_ps(sign_mask, v);
    const __m128 a = _mm_andnot_ps(sign_mask, v);
    const __m128 x = _mm_add_ps(_mm_mul_ps(a, a), a);

    const __m128 y = jxl_fastmath_rational_eval5_x86_sse2(x, k_eotf_p, k_eotf_q);
...
void jxl_color_linear_to_pq_x86_sse2(float *samples, size_t n, float intensity_target) {
    ...
    for (; i + 4 <= n; i += 4) {
        ...
        _mm_storeu_ps(samples + i, jxl_color_linear_to_pq_vec_x86_sse2(v, intensity_target));
    }
...
void jxl_color_pq_to_linear_x86_sse2(float *samples, size_t n, float intensity_target) {
    ...
    for (; i + 4 <= n; i += 4) {
        const __m128 v = _mm_loadu_ps(samples + i);
        _mm_storeu_ps(samples + i, jxl_color_pq_to_linear_vec_x86_sse2(v, intensity_target));
    }
