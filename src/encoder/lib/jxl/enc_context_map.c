...
static jxl_status jxl_move_to_front_transform_body(const jxl_array_u8* v, jxl_array_u8* result,
                                jxl_array_u8* mtf) {
  jxl_array_clear(result);
  if (jxl_array_empty(v)) return jxl_ok_status();
...
static jxl_status jxl_encode_context_map_simple_body(void* opaque) {
...
static jxl_status jxl_encode_context_map_ans_body(void* opaque) {
...
static jxl_status jxl_encode_block_ctx_map_body(void* opaque) {
...
jxl_status jxl_encode_context_map_body_inner(const jxl_array_u8* context_map,
...
if (!jxl_status_ok(jxl_array_token_push_back(jxl_token_streams_at(mtf_tokens, 0), jxl_token_make(0, *jxl_array_at(transformed_symbols, sym_i))))) JXL_CRASH();
}
  jxl_histogram_params params;
...
jxl_status jxl_encode_context_map_body(const jxl_array_u8* context_map,
                            size_t num_histograms, jxl_bit_writer* writer,
                            jxl_layer_type layer, jxl_token_streams* tokens,
                            jxl_token_streams* mtf_tokens) {
  ...
  jxl_status status = jxl_encode_context_map_body_inner(
...
jxl_status jxl_encode_context_map(const jxl_array_u8* context_map,
                        size_t num_histograms, jxl_bit_writer* writer,
                        jxl_layer_type layer){
  if (num_histograms == 1) {
    // Simple code
