...
jxl_status jxl_channel_create(jxl_memory_manager *memory_manager, size_t iw, size_t ih,
  ...
  jxl_status status = jxl_image_i_create(memory_manager, iw, ih, 0, &plane);
...
void jxl_image_init(jxl_image *self, jxl_memory_manager *memory_manager) {
  ...
  self->channel.memory_manager = memory_manager;
}
...
jxl_status jxl_image_create(jxl_memory_manager *memory_manager, size_t iw, size_t ih,
  ...
  jxl_image_init_dims(&result, memory_manager, iw, ih, bitdepth);
  for (int i = 0; i < nb_chans; i++) {
    jxl_channel c;
    jxl_channel_construct_empty(&c);
    jxl_status status = jxl_channel_create(memory_manager, iw, ih, 0, 0, &c);
    if (!jxl_status_ok(status)) {
