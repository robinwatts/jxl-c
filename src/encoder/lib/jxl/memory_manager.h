 ...
 */
typedef void* (*jxl_alloc_func)(void* opaque, size_t size);
 ...
 *
 * @param opaque custom memory manager handle provided by the caller.
 * @param address memory region pointer returned by ::jxl_alloc_func, or @c
 * NULL.
 */
typedef void (*jxl_free_func)(void* opaque, void* address);
 ...
 * allocations.
 */
typedef struct jxl_memory_manager_struct {
