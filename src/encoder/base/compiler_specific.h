...
#include "base/sanitizer_definitions.h"
...
/* C99-compatible alignment attribute (replaces C11 alignas). */
#if JXL_COMPILER_MSVC
#define JXL_ALIGNAS(N) __declspec(align(N))
#elif JXL_COMPILER_GCC || JXL_COMPILER_CLANG
#define JXL_ALIGNAS(N) __attribute__((aligned(N)))
#else
#define JXL_ALIGNAS(N)
#endif
...
#if defined(__cplusplus) && JXL_HAVE_ATTRIBUTE(nodiscard)
...
// Compile-time assertion (C99-compatible; C++ uses static_assert).
#ifdef __cplusplus
#define JXL_STATIC_ASSERT(condition, message) static_assert(condition, message)
#else
#define JXL_STATIC_ASSERT(condition, message)                                  \
  typedef char JXL_STATIC_ASSERT_NAME(__LINE__)[(condition) ? 1 : -1]
#define JXL_STATIC_ASSERT_NAME2(line) jxl_static_assert_##line
#define JXL_STATIC_ASSERT_NAME1(line) JXL_STATIC_ASSERT_NAME2(line)
