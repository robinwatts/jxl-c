...
#ifndef JXL_ENC_ANS_COMMON_H_
#define JXL_ENC_ANS_COMMON_H_
...
jxl_get_population_count_precision(uint32_t logcount, uint32_t shift) {
  int32_t r = JXL_MIN(
      (int32_t)(logcount),
      (int32_t)(shift) -
          (int32_t)((ANS_LOG_TAB_SIZE - logcount) >> 1));
  if (r < 0) return 0;
  return r;
}
...
// of the histogram.
static JXL_MAYBE_UNUSED JXL_INLINE jxl_enc_status
jxl_create_flat_histogram(jxl_array_i32* result, int length, int total_count,
  ...
  JXL_RETURN_IF_ERROR(
      jxl_array_i32_resize_fill(result, (size_t)(length), count));
  const int rem_counts = total_count % length;
  for (int i = 0; i < rem_counts; ++i) {
    ++*jxl_array_at(result, i);
  }
...
typedef struct jxl_alias_table_symbol {
  size_t value;
  size_t offset;
  size_t freq;
} jxl_alias_table_symbol;
...
#pragma pack(push, 1)
typedef struct jxl_alias_table_entry {
  uint8_t cutoff;       // < kEntrySizeMinus1 when used by ANS.
  uint8_t right_value;  // < alphabet size.
  uint16_t freq0;

  // Only used if `greater` (see jxl_alias_table_lookup)
  uint16_t offsets1;         // <= ANS_TAB_SIZE
  uint16_t freq1_xor_freq0;  // for branchless ternary in jxl_alias_table_lookup
} jxl_alias_table_entry;
#pragma pack(pop)
...
static JXL_INLINE jxl_alias_table_symbol jxl_alias_table_lookup(
    const jxl_alias_table_entry* JXL_RESTRICT table, size_t value,
    size_t log_entry_size, size_t entry_size_minus_1) {
