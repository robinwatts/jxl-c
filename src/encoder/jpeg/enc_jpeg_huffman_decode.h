...
enum { kJpegHuffmanRootTableBits = 8 };
...
enum { kJpegHuffmanLutSize = 1024 };

typedef struct jxl_huffman_table_entry {
  // Initialize the value to an invalid symbol so that we can recognize it
  // when reading the bit stream using a Huffman code with space > 0.
  uint8_t bits;    // number of bits used for this symbol
  uint16_t value;  // symbol value or table offset
} jxl_huffman_table_entry;

static inline jxl_huffman_table_entry jxl_huffman_table_entry_make() {
  jxl_huffman_table_entry self;
  self.bits = 0;
  self.value = 0xffff;
  return self;
}

JXL_DEFINE_POD_ARRAY(jxl_array_huffman_table_entry, jxl_huffman_table_entry)
...
void jxl_build_jpeg_huffman_table(const uint32_t* counts, const uint32_t* symbols,
