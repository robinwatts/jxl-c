...
// processed symbol.
static inline int jxl_next_table_bit_size(const int* count, int len) {
  int left = 1 << (len - kJpegHuffmanRootTableBits);
  while (len < (int)(kJpegHuffmanMaxBitLength)) {
    left -= count[len];
    if (left <= 0) break;
    ++len;
    left <<= 1;
...
void jxl_build_jpeg_huffman_table(const uint32_t* count, const uint32_t* symbols,
                           jxl_huffman_table_entry* lut) {
  jxl_huffman_table_entry code;    // current table entry
  jxl_huffman_table_entry* table;  // next available space in table
  ...
  int total_count = 0;
  for (len = 1; len <= (int)(kJpegHuffmanMaxBitLength); ++len) {
  ...
  for (len = kJpegHuffmanRootTableBits + 1;
       len <= (int)(kJpegHuffmanMaxBitLength); ++len) {
    for (; tmp_count[len] > 0; --tmp_count[len]) {
      // Start a new sub-table if the previous one is full.
      if (low >= table_size) {
        table += table_size;
        table_bits = jxl_next_table_bit_size(tmp_count, len);
        table_size = 1 << table_bits;
        low = 0;
