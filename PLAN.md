...
2. **Optional cohesion polish:** CMS color-encoding peel strategy; rename `memory_manager_internal.*` if desired; further Medium shares (ICC shuffle/linear predict, hybrid-uint adapters).
3. **v2+ product:** thread pool (`JXL_C_ENABLE_THREADS`), progressive animation streaming API.

**Complete:** `jxl_context` plan; one-library public headers / status / thin context / install packaging / encoder flatten; shared encoder helpers; `src/common/` ANS params, LZ77 special distances, pack/unpack signed, ICC wire params / ANS context / header predict.
